...
#  From: https://github.com/lmb-freiburg/demon/blob/master/python/depthmotionnet/evaluation/metrics.py
#
...
def torch_l1_inverse(depth1, depth2):
    return torch.nn.functional.l1_loss(torch.reciprocal(depth1), torch.reciprocal(depth2))
...
def rmse_log(depth1, depth2):
    ...
    """
    assert (np.all(np.isfinite(depth1) & np.isfinite(depth2)))
...
def torch_rmse(depth1, depth2):
    return torch.sqrt(torch.nn.functional.mse_loss(depth1, depth2))

def torch_rmse_log(depth1, depth2):
    log_depth1 = torch.log(depth1)
    log_depth2 = torch.log(depth2)

    # print("Log result: ", torch.mean(log_depth1), torch.mean(log_depth2))
    return torch.sqrt(torch.nn.functional.mse_loss(log_depth1, log_depth2))
